...
<svg class="nandi-logo" viewBox="0 0 160 160" xmlns="http://www.w3.org/2000/svg">
  <defs>
    <linearGradient id="g" x1="0" y1="0" x2="1" y2="1">
      ...
      <stop offset="55%" stop-color="#07883f"/>
      <stop offset="100%" stop-color="#034f28"/>
    </linearGradient>
  </defs>
  <path d="M34 20 L62 55 L50 70 L14 35 Z" fill="url(#g)"/>
  <path d="M126 20 L98 55 L110 70 L146 35 Z" fill="url(#g)"/>
  <path d="M28 30 C10 28 8 12 8 6 C28 16 44 24 58 45 Z" fill="#07983f"/>
  <path d="M132 30 C150 28 152 12 152 6 C132 16 116 24 102 45 Z" fill="#07983f"/>
  <path d="M38 58 L80 30 L122 58 L109 112 L80 145 L51 112 Z" fill="url(#g)"/>
  <path d="M54 66 L80 46 L106 66 L96 101 L80 118 L64 101 Z" fill="#055c2d" opacity="0.55"/>
  <path d="M58 79 L72 85 L63 95 Z" fill="#eafff2"/>
  <path d="M102 79 L88 85 L97 95 Z" fill="#eafff2"/>
  <path d="M70 111 L80 128 L90 111 Z" fill="#02391d"/>
</svg>
"""
...
<style>
    .stApp {
        background:
            ...
            radial-gradient(circle at bottom right, rgba(7, 136, 63, 0.12), transparent 34%),
            linear-gradient(135deg, #ffffff 0%, #f7fff9 45%, #eefaf2 100%);
        color: #101820;
    }
    ...
    .nandi-logo {
        width: 82px;
        height: 82px;
        filter: drop-shadow(0 14px 24px rgba(0,120,55,0.22));
    }
    ...
    .dot {
        ...
        box-shadow: 0 0 0 5px rgba(13,174,85,0.12);
    }
    ...
    .card {
        ...
        border-radius: 24px;
        padding: 22px;
        box-shadow: 0 12px 42px rgba(7, 90, 44, 0.08);
        min-height: 215px;
    }
    ...
    .small-card {
        ...
        border-radius: 20px;
        padding: 16px;
        box-shadow: 0 9px 30px rgba(7, 90, 44, 0.07);
    }
    ...
    .progress-fill {
        height: 100%;
        border-radius: 999px;
        background: linear-gradient(90deg, #07983f, #00c853);
    }
    ...
    div[data-testid="stForm"] {
        ...
        border-radius: 30px;
        padding: 35px;
        box-shadow: 0 18px 60px rgba(7,90,44,0.12);
    }
...
if "watchlist" not in st.session_state:
    st.session_state.watchlist = ["NIFTY 50", "BANK NIFTY", "FINNIFTY", "INDIA VIX", "USD/INR", "GOLD", "CRUDE OIL"]
...
def candle_chart(values, width=620, height=255):
    ...
    for i in range(len(values)):
        x = i * step + step / 2
        yo, yc, yh, yl = y(opens[i]), y(values[i]), y(highs[i]), y(lows[i])
        up = values[i] >= opens[i]
...
def login_page():
    ...
    with left:
        ...
        <div class="card">
            <div class="card-title">Market Pulse <span class="tag" style="float:right;">Live</span></div>
        ...
        st.markdown(f"""
        <div class="card">
            <div class="card-title">Finance Research Preview <span class="tag" style="float:right;">NIFTY 15m</span></div>
            {candle_chart(prices)}
            <div class="row">
    ...
    with right:
        ...
        <div style="text-align:center;margin-bottom:18px;">
            {BULL_LOGO}
            <h1 style="margin:6px 0 4px 0;font-size:34px;letter-spacing:-1px;">Welcome back 👋</h1>
...
def command_center():
    ...
    with top_left:
        st.text_input("Search", placeholder="Search markets, research, tickers, or ideas...", label_visibility="collapsed")
    with top_right:
        st.markdown(f"""
    ...
    with c2:
        st.markdown("""
        <div class="card">
            <div class="card-title">Research Engine <span class="tag" style="float:right;">Active</span></div>
            <div class="gauge">
    ...
    with c3:
        st.markdown("""
        <div class="card">
            <div class="card-title">Memory Core <span class="tag" style="float:right;">Learning</span></div>
    ...
    with c4:
        st.markdown("""
        <div class="card">
            <div class="card-title">Today's Focus <span class="tag" style="float:right;">60%</span></div>
    ...
    with left:
        st.markdown(f"""
        <div class="card">
            <div class="card-title">Finance Research Panel <span class="tag" style="float:right;">NIFTY 50 · 15m</span></div>
    ...
    with right:
        st.markdown("""
        <div class="card">
            <div class="card-title">Paper Journal / Backtests <span class="tag" style="float:right;">View All</span></div>
            <table style="width:100%;border-collapse:collapse;font-size:13px;">
                <tr>
                    <th align="left">Title</th><th align="left">Type</th><th align="left">Result</th><th align="left">Score</th>
                </tr>
                <tr><td>NIFTY Breakout Study</td><td>Backtest</td><td class="green">+1.34%</td><td><span class="tag">78/100</span></td></tr>
                <tr><td>Bank Nifty Call Study</td><td>Backtest</td><td class="green">+0.92%</td><td><span class="tag">72/100</span></td></tr>
                <tr><td>Opening Range Momentum</td><td>Journal</td><td class="green">+0.68%</td><td><span class="tag">65/100</span></td></tr>
                <tr><td>Midcap Rotation Scan</td><td>Backtest</td><td class="red">-0.21%</td><td><span class="tag tag-red">52/100</span></td></tr>
                <tr><td>Global Cues Impact</td><td>Journal</td><td class="green">+0.44%</td><td><span class="tag">60/100</span></td></tr>
            </table>
        </div>
        """, unsafe_allow_html=True)
    ...
    html = '<div class="small-card"><div class="card-title">Market Watch <span class="tag" style="float:right;">Customize</span></div><div class="ticker-grid">'
    for name, price, move, color, seed in tickers:
        data = make_series(seed=seed, points=28, start=100)
        ...
        <div class="ticker-card">
            ...
            {sparkline(data, color=color, width=140, height=45)}
        </div>
        """
    html += "</div></div>"
...
if not st.session_state.logged_in:
    ...
    if st.session_state.page == "Command Center":
        command_center()
    elif st.session_state.page == "Finance Research":
        finance_research()
    elif st.session_state.page == "Nandi Chat":
        nandi_chat()
    elif st.session_state.page == "Memory Core":
        memory_core()
    elif st.session_state.page == "Goals":
        goals()
    elif st.session_state.page == "Daily Updates":
        daily_updates()
