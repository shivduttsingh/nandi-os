...
class SMTPEmailAlertSink:
    ...
    def send_decision(self, decision: Decision, spot: float, expiry: str) -> EmailDelivery:
        ...
        message.set_content("\n".join([
            ...
            f"Contract: NIFTY {decision.selected_strike:.0f} {decision.side}" if decision.selected_strike else "Contract: —",
            f"Option LTP: {levels.option_ltp:.2f}" if levels.option_ltp is not None else "Option LTP: —",
            f"Reference option entry: {levels.option_entry:.2f}" if levels.option_entry is not None else "Reference option entry: —",
            f"Option stop-loss (5%): {levels.option_stop:.2f}" if levels.option_stop is not None else "Option stop-loss: —",
            f"Option target 1 (1.5R): {levels.option_target_1:.2f}" if levels.option_target_1 is not None else "Option target 1: —",
            f"Option target 2 (2.5R): {levels.option_target_2:.2f}" if levels.option_target_2 is not None else "Option target 2: —", "",
            f"NIFTY spot entry: {levels.entry:.2f}" if levels.entry is not None else "NIFTY spot entry: —",
            f"Spot stop-loss: {levels.stop:.2f}" if levels.stop is not None else "Spot stop-loss: —",
            f"Spot target 1: {levels.target_1:.2f}" if levels.target_1 is not None else "Spot target 1: —",
            f"Spot target 2: {levels.target_2:.2f}" if levels.target_2 is not None else "Spot target 2: —",
