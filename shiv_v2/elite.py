...
def assess_a_plus_plus_plus_plus(
    ...
    *,
    ...
    """Ultra-selective research gate; never a guarantee of the next trade."""
    blockers: list[str] = []
    selected = decision.strike_selection.selected

    if not decision.actionable:
        blockers.append("V2 is not actionable with ENTRY READY.")
    if decision.setup_quality < max(90.0, decision.required_quality + 8.0):
        blockers.append("Setup quality has not cleared the elite margin.")
    if decision.base.mtf_agreement < max(90.0, decision.required_mtf + 5.0):
        blockers.append("Multi-timeframe agreement is below the elite threshold.")
    ...
    if selected is None:
        blockers.append("No option contract has passed selection.")
    else:
    ...
    if decision.breakout.blocked:
        blockers.append("False-breakout protection is blocking the setup.")
    ...
    if decision.session.bucket not in {SessionBucket.MORNING, SessionBucket.AFTERNOON}:
        blockers.append("A++++ is restricted to the cleaner morning/afternoon session windows.")
    ...
    if decision.pattern.confirmed and decision.pattern.side not in {"", "NONE", decision.side}:
        blockers.append("Confirmed M/W pattern conflicts with the trade side.")
    ...
    if similarity.win_rate is None or similarity.win_rate < minimum_observed_win_rate:
        historical_blockers.append(f"Comparable observed win rate must be at least {minimum_observed_win_rate:.0f}%.")
    ...
    if walk_forward.selected_trades < minimum_walk_forward_trades:
        historical_blockers.append(f"Needs at least {minimum_walk_forward_trades} selected walk-forward test trades.")
    ...
    if validated:
        ...
        reason = (
            "Ultra-selective live evidence is aligned and completed comparable/walk-forward samples clear the validation gates. "
            "Historical rates are observations, not a guarantee for the next trade."
        )
    elif live_candidate:
        status = f"A++++ CANDIDATE {decision.side} — EVIDENCE BUILDING"
        reason = "The live setup clears the ultra-selective market gates, but historical validation is not strong enough yet."
    else:
        status = "A++++ LOCKED"
        reason = "One or more elite live-market gates are not satisfied, so Shiv does not promote this setup to A++++."
